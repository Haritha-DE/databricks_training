...
# MAGIC %sql
# MAGIC describe extended silverTable
...
# MAGIC %sql
# MAGIC select * from table_changes('silverTable',2)
...
# MAGIC %sql
# MAGIC create or replace temporary view silverTable_lastest as 
# MAGIC select * from 
# MAGIC (select *,rank() over (partition by Country order by _commit_version desc) as rank from table_changes('silvertable',2,5) where _change_type!='update_preimage')
# MAGIC where rank= 1;
